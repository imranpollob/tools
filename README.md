...
Edit `src/tools.json`. Every entry has a repository, title, short subtitle, category, availability (`online` or `install`), destination URL, action label, preview filename, background color, and optional search tags. Put images in `public/previews/`.
